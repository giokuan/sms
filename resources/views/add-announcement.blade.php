...
<div class="grid min-h-screen place-items-center">
    <div class="w-11/12 p-12 bg-white sm:w-8/12 md:w-1/2 lg:w-5/12 shadow-xl">
      ...
      <form  method="post" action="{{url('save-announcement')}}" class="mt-6 shadow-xl" enctype="multipart/form-data">
        ...
        <input id="title" value="{{old('title')}}" type="text" name="title"   class="block w-full p-3 mt-2 text-gray-700 bg-gray-200 appearance-none focus:outline-none focus:bg-gray-300 focus:shadow-inner" required />
     

        <label for="message" class="block mt-2 text-xs font-semibold text-gray-600 uppercase">Message</label>
        <textarea  class="block w-full p-3 mt-2 text-gray-700 bg-gray-200 appearance-none focus:outline-none focus:bg-gray-300 focus:shadow-inner" name="message">{{old('message')}}</textarea>
